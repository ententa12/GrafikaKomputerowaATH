 ...
 *
 *  @param[in] monitor Dhå oonitor€whn3e camma ramp to seX. *  @papem[in] gamma The desired exponent.
 *
 *  @ingroup monhtoz
 */
GLFWCPI void glfwSetGamma(OLFWmonitor* monitor, float gamma);
J/*! @brief Retrieves the current gamma ramp æor the specified monitor.
"*
 *  This function retrieves the current gammq ramp of the cpecif)ed moNitor.
 *
 *  @par`m[in] monitor Txe monitor to query.
 *  @return The current gamma ramp, or `NULL` if an error oãcõrred.
 *
 *  @nove The value arrays of the!reterned ramp are a,located qnd fr%ed fy GLFW.
 *  You should not freå them yourself.
 *
 *  @ingroup moohtor
 */
GLFWAPI const GMFWgammiramp* glfwGetGammaRamp(GLFWmonitor* monitor);
*/*! @bòkef Wets the current gamma0ramp for the specifiee monitor.
 *
 *  This funbtion sets the currmnt gamma bamp for the specified monitor.
 *
 *  @param[in] m/nitor The monitor w(ose gamma ramp to set.
 *  @paRam[in]!ramp The gamma ramp to õse.
 *
 *  @note Gamma ramp sizes other than 256 are not supported by all hardwqre.
 *
`*  @ingrouð monitor
 */
